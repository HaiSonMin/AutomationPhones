...
from constants.ConstantPermission import (
    PERMISSION_CANVA,
...
def turn_on_permission_app_device(deviceKey: str, appName: str):
    ...
    if appName == EAppNamePermission.Canva.value:
        for permission in PERMISSION_CANVA:
            ...
            command = f"adb -s {deviceKey} shell pm grant {ConstantPackageApplication.CANVA} {permission}"
            subprocess.run(command, shell=True, capture_output=True, text=True)
    ...
    if appName == EAppNamePermission.Chrome.value:
        for permission in PERMISSION_CHROME:
            ...
            command = f"adb -s {deviceKey} shell pm grant {ConstantPackageApplication.CHROME} {permission}"
            subprocess.run(command, shell=True, capture_output=True, text=True)
    ...
    if appName == EAppNamePermission.CloneAppPro.value:
        for permission in PERMISSION_CLONE_APP_PRO:
            ...
            command = f"adb -s {deviceKey} shell pm grant {ConstantPackageApplication.CLONE_APP_PRO} {permission}"
            subprocess.run(command, shell=True, capture_output=True, text=True)
    ...
    if appName == EAppNamePermission.Gmail.value:
        for permission in PERMISSION_GMAIL:
            ...
            command = f"adb -s {deviceKey} shell pm grant {ConstantPackageApplication.GMAIL} {permission}"
            subprocess.run(command, shell=True, capture_output=True, text=True)
    ...
    if appName == EAppNamePermission.PlayStore.value:
        for permission in PERMISSION_PLAY_STORE:
            ...
            command = f"adb -s {deviceKey} shell pm grant {ConstantPackageApplication.PLAY_STORE} {permission}"
            subprocess.run(command, shell=True, capture_output=True, text=True)
